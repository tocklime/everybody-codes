...
fn solve2(input: &str) -> usize {
    ...
    let interesting = g
        .indexed_iter()
        .filter(|(p, b)| p == &start || b.is_alphabetic())
        .collect_vec();
    let mut lookup: HashMap<Point<usize>, HashMap<Point<usize>, usize>> = HashMap::new();
    for a in interesting.iter().combinations(2) {
        let b = a[1].0;
        let a = a[0].0;
        let dist = pathfinding::directed::astar::astar(
            &a,
            |p| {
                g.neighbours(*p)
                    .filter(|n| !"#~".contains(g[*n]))
                    .map(|n| (n, 1))
            },
            |x| x.manhattan_unsigned(&b),
            |x| *x == b,
        );
        let d = dist.unwrap().1;
    ...
    let path = pathfinding::directed::astar::astar(
        ...
        |(x, got)| {
            let valid_targets = if got == &all_herbs {
                ...
                interesting
                    .iter()
                    .filter(|x| x.1.is_alphabetic() && !got.contains((*x.1 as u8) - b'A'))
                    .map(|x| (x.0, *x.1))
                    .collect()
            };
            valid_targets
        ...
        |(pos, got)| {
            if got == &all_herbs {
                ...
                let (there, last_point) = missing
                    .iter()
                    .map(|i| {
                        herb_to_locs[&i]
                            .iter()
                            .map(|p| (lookup[pos][p], p))
                            .min()
                            .unwrap()

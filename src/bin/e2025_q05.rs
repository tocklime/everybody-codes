...
fn concat_digits<T: IntoIterator<Item = u64>>(t: T) -> u64 {
    t.into_iter().map(|x| x.to_string()).join("").parse().unwrap()
}
...
impl Segment {
    ...
    fn level_number(&self) -> u64 {
        concat_digits([self.left, Some(self.spine), self.right].iter().filter_map(|x| *x))
    }
}
struct Sword {
    id: u64,
...
impl Sword {
    fn parse(input: &str) -> Self {
        let (id, ns) = input.split_once(':').unwrap();
        let ns : Vec<u64> = ns.split(',').map(|x| str::parse::<u64>(x).unwrap()).collect();
        ...
        Self { id: id.parse().unwrap(), segments}
    }
    fn make_cmp_key(&self) -> (u64, Vec<u64>, u64) {
        (self.quality(), self.segments.iter().map(|x| x.level_number()).collect(), self.id)
    }
    fn quality(&self) -> u64 {
...
fn assess_sword(input: &str) -> u64 {
    Sword::parse(input).quality()
}
fn get_checksum(input: &str) -> u64 {
    let mut swords : Vec<Sword> = input.lines().map(Sword::parse).collect();
    swords.sort_by_cached_key(|x| Reverse(x.make_cmp_key()));
    let ids = swords.iter().map(|x| x.id).collect_vec();
    ids.iter().zip(1..).map(|(id, ix)| id * ix).sum()

}

fn find_quality_range(input: &str) -> u64 {
    let x = input.lines().map(assess_sword).minmax();
    match x {

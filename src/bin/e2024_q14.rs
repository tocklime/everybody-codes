...
fn grow(input: &str) -> impl Iterator<Item=(i64,i64,i64)> + use<'_> {
    let mut st = (0,0,0);
...
fn neighbours(p: C, seen: &HashSet<C>) -> Vec<(C,u64)> {
    [((p.0-1,p.1,p.2),1),
     ...
     ((p.0,p.1,p.2-1),1),
     ((p.0,p.1,p.2+1),1),
    ].into_iter().filter(|p| seen.contains(&p.0)).collect()
}
...
fn solve3(input: &str) -> u64 {
    ...
    (0..=max_height).map(|i| {
        ...
        if seen.contains(&p) {
            leaves.iter().map(|l| {
                let path = pathfinding::directed::astar::astar(&p, |x| neighbours(*x,&seen), |x| hamming(*l,*x), |x| x == l);
                path.unwrap().1
            }).sum::<u64>()
        } else {
            u64::MAX

...
fn solve<const PART: usize>(input: &str) -> isize {
    ...
    let catapults = g
        .indexed_iter()
        .filter(|x| "ABC".contains(*x.1))
        .map(|x| (x.0.into(), x.1))
        .collect_vec();
    //natural order of targets here is good: top to bottom, left to right.
    let targets = g
        .indexed_iter()
        .filter(|x| "TH".contains(*x.1))
        .map(|x| (x.0.into(), x.1, false))
        .collect_vec();
    let mut ans = 0;
    let max_target_x: isize = targets
        .iter()
        .map(|x: &(Point<isize>, &char, bool)| x.0.x)
        .max()
        .unwrap();
    let min_catapult_x: isize = catapults
        .iter()
        .map(|x: &(Point<isize>, &char)| x.0.x)
        .min()
        .unwrap();
